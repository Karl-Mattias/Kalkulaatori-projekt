...
def plot(func, min_x=-10, min_y=-10, max_x=10, max_y=10, scale=20):
    ...
    #vastavalt skaalale ja määratud maksimaalsele x'i ja y'i väärtusele
    #määran graafiku akna suuruse

    width = (abs(min_x)+abs(max_x))*scale
    height = (abs(max_y)+abs(min_y))*scale
    setup(width,height)

    #Nüüd leian graafiku telgede asukohad. Kuna y suureneb ülevalt alla,
    #siis tuleb seal maksimumist hoopis lahutada miinumum

    kesk_x = ((abs(min_x)-abs(max_x))*scale+width)/2
    kesk_y = ((abs(max_y)-abs(min_y))*scale+height)/2

    #Tsükkel joonistab kui funktsioon on täidetud ja peale joonistab teljed

    for e in range(width):
        for i in range(height):
            see=func.replace('x','('+str((e - kesk_x)/scale)+')').replace('y','('+str((i - kesk_y)/(-scale))+')')
            if eval(see):
                set_pixel(e,i,'blue')
